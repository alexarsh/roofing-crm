...
5. Present results as a compact markdown table (address, parcel id, roof age + basis, permit status, days/years open, contractor, BBB rating or "not available", distance) followed by: Assumptions, Missing data (e.g. BBB not matched, contractor not recorded, roof age unknown), and Sources (cite source_urls / source_url values from the rows).
